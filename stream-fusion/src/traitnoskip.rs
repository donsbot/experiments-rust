            ...
            match stream1.next() {
                ...
                Step::Yield(x, s) => {
                    if p(&x) {
                        ...
                        return Step::Yield (x, s1)
                    } else {
                        stream1 = s;
                        // In the paper we would just Skip here, to keep it non-recursive.
                        // Without skip, we need a nested loop
                        // Step::Skip(s1), need to recurse.
                    }
                }
            }
        }
